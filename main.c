...
#define MIC_CH     2
#define JOY_X_PIN  26      // ADC0  (eixo X)
#define JOY_X_CH    0
#define PWM_PIN    21
#define BTN_A       5
#define BTN_B       6
...
#define MAX_SECS      15u
#define MIN_SECS       1u
#define DEF_SECS       5u           // padrão de 5 s
#define BUF_SAMPLES (FS_HZ*MAX_SECS)

#define US_STEP (1000000u / FS_HZ)  // 125 µs para 8 kHz
...
static bool rec_cb(struct repeating_timer *t){
    ...
    adc_select_input(MIC_CH);            // <-- garante canal do microfone
    buf[rec_i++] = adc_read();
    return true;
}
...
static void joystick_update(void){
    if(st!=IDLE) return;                 // só ajusta em repouso

    adc_select_input(JOY_X_CH);          // lê eixo X
...
int main(void){
    ...
    while(1){
        joystick_update();              // ajuste de segundos
